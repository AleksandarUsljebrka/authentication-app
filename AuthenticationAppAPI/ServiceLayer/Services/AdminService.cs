...
namespace ServiceLayer.Services
{
	public class AdminService:IAdminService
	{
		...
		public async Task<IResult> GetAllUsers(string token, PaginationDto paginationDto)
		{
		...
		public async Task<IResult> SearchUserByEmail(string email, string token)
		{
			var admin = await _tokenHelper.UserByToken(token);
			if (admin is null) return new Result(false, ErrorCode.NotFound, "Admin not found");
			...
			var user = _userManager.Users.Where(u => u.Email == email && !u.IsDeleted).FirstOrDefault();
